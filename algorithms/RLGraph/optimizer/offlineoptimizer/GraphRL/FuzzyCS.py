...
class FuzzyCS:
    ...
    def initialize(self):
        ...
        U_2 = FuzzySet(function=Trapezoidal_MF(a=0.15, b=0.45, c=0.55, d=0.85), term='moderate')
        U_3 = FuzzySet(function=Trapezoidal_MF(a=0.55, b=0.85, c=1, d=1), term='high')
        LV1 = LinguisticVariable([U_1, U_2, U_3], concept='U', universe_of_discourse=[0, 1])
        FS.add_linguistic_variable('U', LV1)

        ESR_1 = FuzzySet(function=Trapezoidal_MF(a=0, b=0, c=0.1, d=0.2), term='low')
        ESR_2 = FuzzySet(function=Trapezoidal_MF(a=0.1, b=0.2, c=0.5, d=0.6), term='moderate')
        ESR_3 = FuzzySet(function=Trapezoidal_MF(a=0.5, b=0.6, c=1, d=1), term='high')
        ...
        E_2 = FuzzySet(function=Trapezoidal_MF(a=0.15, b=0.45, c=0.55, d=0.85), term='moderate')
        E_3 = FuzzySet(function=Trapezoidal_MF(a=0.55, b=0.85, c=1, d=1), term='high')
        ...
        R2 = 'IF (U IS low) AND (ESR IS low) AND (E IS moderate) THEN (SAFE IS verylow)'
        R3 = 'IF (U IS low) AND (ESR IS low) AND (E IS high) THEN (SAFE IS low)'
        ...
        R10 = 'IF (U IS moderate) AND (ESR IS low) AND (E IS low) THEN (SAFE IS moderate)'
        R11 = 'IF (U IS moderate) AND (ESR IS low) AND (E IS moderate) THEN (SAFE IS moderate)'
        R12 = 'IF (U IS moderate) AND (ESR IS low) AND (E IS high) THEN (SAFE IS moderate)'
        ...
        R19 = 'IF (U IS high) AND (ESR IS low) AND (E IS low) THEN (SAFE IS moderate)'
        R20 = 'IF (U IS high) AND (ESR IS low) AND (E IS moderate) THEN (SAFE IS high)'
        R21 = 'IF (U IS high) AND (ESR IS low) AND (E IS high) THEN (SAFE IS high)'
